...
class WebhookRequestSpec extends FlatSpec with Matchers {
  ".parse" should "return an WebhookRequest if line match log pattern " in {
    ...
    WebhookRequest.parse(line) should contain (WebhookRequest("https://woodenoyster.com.br", 503))
  }

  it should "return None when it's not able to parse" in {
    WebhookRequest.parse("""Lorem ipsum dolor""") shouldBe empty
  }
}
